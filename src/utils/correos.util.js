...
<div style="font-family:Nunito,Arial,sans-serif;background:#f4f4f4;padding:40px 0;">
  ...
  /* ╔══════════════════════════════════════════════╗
    ║ 6. RECORDATORIO DE CITA (20 MINUTOS ANTES)   ║
    ╚══════════════════════════════════════════════╝ */
  recordatorioCita({ destinatario, cliente_nombre, barbero_nombre, fecha_hora, servicio_nombre }) {
    const fechaFormateada = format(fecha_hora, "d 'de' MMMM 'de' yyyy", { locale: es });
    const horaFormateada = format(fecha_hora, "hh:mm a", { locale: es });

    if (destinatario === 'barbero') {
      return `
      <div style="font-family:Nunito,Arial,sans-serif;background:#f4f4f4;padding:40px 0;">
        <table align="center" cellpadding="0" cellspacing="0" style="max-width:500px;background:#ffffff;padding:32px 24px;border-radius:8px;border:3px solid #FFD700;">
          <tr>
            <td style="text-align:center;padding:20px;">
          ...
          <tr>
            <td style="padding:20px;background:#FFF9E6;border-radius:8px;">
              <h2 style="font-size:24px;margin:0 0 10px;color:#FF6B00;">⏰ Recordatorio de Cita</h2>
              <p style="margin:10px 0;font-size:16px;"><strong>¡Tu próxima cita es en 20 minutos!</strong></p>
